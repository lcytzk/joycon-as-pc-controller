...
public class Controller {
    ...
    /// Approximate count of standard reports that never arrived, derived from
    /// the timer byte each report carries: it advances once per 5ms IMU sample,
    /// so a normal report steps it by 3 and a larger step means the report(s)
    /// in between were lost on the way. Distinguishes a flaky Bluetooth link
    /// from a reader that isn't keeping up.
    public internal(set) var missedSensorFrames: Int = 0
    private var lastReportTimer: UInt8?
    ...
    func readSensorData(value: IOHIDValue) {
        ...
        let timer = ptr.pointee
        if let last = self.lastReportTimer {
            let delta = Int(timer &- last)
            if delta > 3 && delta < 128 {
                self.missedSensorFrames += (delta - 3) / 3
            }
        }
        self.lastReportTimer = timer
