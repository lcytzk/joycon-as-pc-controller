...
public class JoyConManager {
    ...
    public func run() -> IOReturn {
        ...
        IOHIDManagerScheduleWithRunLoop(self.manager, runLoop.getCFRunLoop(), CFRunLoopMode.commonModes.rawValue)
        ...
        IOHIDManagerUnscheduleFromRunLoop(self.manager, runLoop.getCFRunLoop(), CFRunLoopMode.commonModes.rawValue)
    ...
    public func runAsync() -> IOReturn {
        // A dedicated thread rather than a global queue: `run()` never returns,
        // so dispatching it would permanently retire a thread from the shared
        // pool. .userInteractive matches the latency sensitivity of live
        // controller input — at default priority this run loop gets
        // deprioritized under load, which shows up as multi-hundred-millisecond
        // gaps in sensor delivery.
        let thread = Thread { [weak self] in
            _ = self?.run()
        }
        thread.name = "JoyConSwift.HID"
